	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		while(result[i][0]!=null)
		{
			text+="<tr><td>"+result[i][0]+"</td><td>"+result[i][1]+"</td><td>"+result[i][2]+"</td><td>"+result[i][3]+"</td><td>"+result[i][4]+"</td><td>"+result[i][5]+
					"</td><td class=\"mybt\"><button id=\"bt"+result[i][0]+"\" value=\""+result[i][0]+ "\" onclick=\"DoTest('"+result[i][0]+"','"+result[i][3]+"')\">Làm bài</button></td>"
			+"<td class=\"mybt2\"><button id=\"bt"+result[i][0]+"\" value=\""+result[i][0]+ "\" onclick=\"ShowTest('"+result[i][0]+"')\">Xem</button></td></tr>";
			i++;
		}
		text+="</table>";
